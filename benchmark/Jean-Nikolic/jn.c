/* Implementation of a MAC based on the the Jean-Nikolic construction
...
void jn_init(context *ctx, const uint8_t k[]) {
  ...
  for (unsigned i=0; i<tabsize(ctx->subkeys); i++)
    ctx->subkeys[i] = AES(Ki, _mm_set_epi64x(1,i));

  AES_KS(AES(Ki, _mm_set_epi64x(2,0)), ctx->keys[0]);
  AES_KS(AES(Ki, _mm_set_epi64x(2,1)), ctx->keys[1]);
}


state jn_AU(context *ctx, const uint8_t *m, size_t mlen) {
  // Padding
  ...
  for (; M<Mfin; M+=3) {
    ...
    S.S[10] =                  S.S[ 9]^M[1];
    S.S[ 9] =                  S.S[ 8]^M[0];
    S.S[ 8] = _mm_aesenc_si128(S.S[ 7],M[2]) ^ S.S[8];
    S.S[ 7] = _mm_aesenc_si128(S.S[ 6],M[2]);
    S.S[ 6] =                  S.S[ 5]^M[2];
    S.S[ 5] = _mm_aesenc_si128(S.S[ 4],M[1]) ^ S.S[5];
    S.S[ 4] = _mm_aesenc_si128(S.S[ 3],M[1]);
    S.S[ 3] =                  S.S[ 2]^M[1];
    S.S[ 2] = _mm_aesenc_si128(S.S[ 1],M[0]) ^ S.S[2];
    S.S[ 1] = _mm_aesenc_si128(S.S[ 0],M[0]);
    S.S[ 0] = T ^ M[0];
  }
  ...
  // Last round (padding)
  {
    ...
    S.S[11] =                  S.S[10]^M_padding[2];
    S.S[10] =                  S.S[ 9]^M_padding[1];
    S.S[ 9] =                  S.S[ 8]^M_padding[0];
    S.S[ 8] = _mm_aesenc_si128(S.S[ 7],M_padding[2]) ^ S.S[8];
    S.S[ 7] = _mm_aesenc_si128(S.S[ 6],M_padding[2]);
    S.S[ 6] =                  S.S[ 5]^M_padding[2];
    S.S[ 5] = _mm_aesenc_si128(S.S[ 4],M_padding[1]) ^ S.S[5];
    S.S[ 4] = _mm_aesenc_si128(S.S[ 3],M_padding[1]);
    S.S[ 3] =                  S.S[ 2]^M_padding[1];
    S.S[ 2] = _mm_aesenc_si128(S.S[ 1],M_padding[0]) ^ S.S[2];
    S.S[ 1] = _mm_aesenc_si128(S.S[ 0],M_padding[0]);
    S.S[ 0] = T ^ M_padding[0];
  }
...
void jn_MAC(context *ctx, const uint8_t *nonce, const uint8_t *m, size_t mlen, uint8_t *tag) {
  state S = jn_AU(ctx, m, mlen);
  ...
  T ^= AES_modified(ctx->subkeys+9, S.S[9]);
  T ^= AES_modified(ctx->subkeys+10, S.S[10]);
  T ^= AES_modified(ctx->subkeys+11, S.S[11]);
