// Reference code from https://eprint.iacr.org/2022/116.pdf
...
void rocca_encrypt(rocca_context * ctx , uint8_t * out , const uint8_t * in , size_t  size) {
  ...
  COPY_FROM_LOCAL(ctx);ctx ->size_m  += size;
}
...
void rocca_decrypt(rocca_context * ctx , uint8_t * out , const uint8_t * in , size_t  size) {
  ...
  COPY_TO_LOCAL(ctx);
  size_t i = 0;
  for(size_t  size2 = FLOORTO(size , ROCCA_MSG_BLOCK_SIZE ); i < size2; i +=  ROCCA_MSG_BLOCK_SIZE) {DECRYPT(out + i, in + i);
  }
  if(i < size) {
