...
#define ROUND(S, M0, M1, M2, M3, R0, R1, R2) do {			\
    __m128i T = S.S[8];						\
...
state lemac_AU(context *ctx, const uint8_t *m, size_t mlen) {
  ...
  // Main rounds
  for (;M < Mfin; M+=4) {
    ROUND(S, M[0], M[1], M[2], M[3], R0, R1, R2);
  }

  // Last round (padding)
  ROUND(S, M_padding[0], M_padding[1], M_padding[2], M_padding[3], R0, R1, R2);

  // Three final rounds to absorb message state
  ROUND(S, STATE_0, STATE_0, STATE_0, STATE_0, R0, R1, R2);
  ROUND(S, STATE_0, STATE_0, STATE_0, STATE_0, R0, R1, R2);
  ROUND(S, STATE_0, STATE_0, STATE_0, STATE_0, R0, R1, R2);
